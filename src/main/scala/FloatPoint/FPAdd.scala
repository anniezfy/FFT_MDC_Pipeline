...
import Chisel._
import Compute.FloatPoint.FloatUtils.getExpMantWidths
import chisel3.{WireInit, getVerilogString}
...
class FPAddStage2(val n: Int) extends Module {
  ...
  val shifted_mant = WireInit(Mux(io.mant_shift > UInt(mantWidth + 1),
...
class FPAdd(val n: Int) extends Module {
  val io = new Bundle {
    val a: UInt = Bits(INPUT, n)
    val b: UInt = Bits(INPUT, n)
    val res: UInt = Bits(OUTPUT, n)
  }
 ...
 */
object FPAdd {
  def apply(n: Int, a: Bits, b: Bits): Bits = {
    val fpAdd = Module(new FPAdd(n))
    fpAdd.io.a := a
    fpAdd.io.b := b
    fpAdd.io.res
  }
...
object FPAdd32App extends App{
  print(getVerilogString(new FPAdd32))
}
