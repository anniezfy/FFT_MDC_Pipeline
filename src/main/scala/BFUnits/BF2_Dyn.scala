...
class BF2_Dyn_blackbox(n:Int) extends Module with HasDataConfig{
  ...
  val twiddle_Address_BF_4_2 = Module(new Twiddle_Address(n))
  twiddle_Address_BF_4_2.io.row := io.row //第四行
  twiddle_Address_BF_4_2.io.ready := Sub.io.valid
  twiddle_Address_BF_4_2.io.counter := 8.U // 存储器中深度为8个

  /*调用计数器*/
  val TF_im_reg = TF_im_ROM(twiddle_Address_BF_4_2.io.address)
  val TF_re_reg = TF_re_ROM(twiddle_Address_BF_4_2.io.address)
